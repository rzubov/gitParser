(function () {
  ...
  function UserController($http, $stateParams, $log, $scope, headerParser) {
    ...
    $http({method: 'GET', url: 'https://api.github.com/users/' + $stateParams.login})
      .success(function (data) {
        ...
        $http({method: 'GET', url: data.repos_url + '?page=1&per_page=1'})
          .success(function (data, status, headers) {
            if (headers('Link')) {
              $scope.totalItems = headerParser(headers('Link')).last;
            }
            function getData() {
              $http.get($scope.reposHost + '?page=' + $scope.currentPage + '&per_page=' + $scope.perPage)
